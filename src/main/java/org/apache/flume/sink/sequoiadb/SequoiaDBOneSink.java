...
public class SequoiaDBOneSink extends AbstractSink implements  Configurable
{
    private static final Logger LOG = LoggerFactory.getLogger(SequoiaDBOneSink.class);
    ...
    private HashSet<String> operationTypes;
    ...
    public void configure(Context context) {
        ...
        if (!operationTypesStr.equals(DEFAULTOPERATIONTYPE)){
        	operationTypes = new HashSet<String>();
        	String []types = operationTypesStr.split(DELIMITER);
        	for (int i = 0; i < types.length; i++) {
        		if(types[i].equals("insert")){
        			operationTypes.add("I");
        		}else if (types[i].equals("update")){
        			operationTypes.add("U");
        		}else if (types[i].equals("delete")){
        			operationTypes.add("D");
        		}
			}
        }else{
	...
	private int drainOne(Channel channel)throws BaseException, InterruptedException{
		int txnEventCount = 0;
        ...
        try {
	    	...
	    	for (; txnEventCount < batchSize; ++txnEventCount) {
	            ...
	            LOG.debug("event body is :"+content);
	            
	            String []infos = content.split(operationDelemiter);
	            ...
	            //判断操作类型是all，还是单个
	            if (defaultOperationType != null && defaultOperationType.equals(DEFAULTOPERATIONTYPE)){
	            	if (operation.equals("I")){
	            		insert(infos[1]); 
		            }else if (operation.equals("U")){
		            	update(infos[1],infos[2]);
		            }else if (operation.equals("D")){
		            	delete(infos[1]);
		            }
	            }else if(operationTypes.contains("I") && operationTypes.size() == 1){ //insert 批量插入
	            	List<BSONObject> items = new ArrayList<BSONObject>();
	            	
	            	BSONObject item = new BasicBSONObject();
	                ...
	                for (int i = 0; i < values.length; i++) {
	        			...
	        			LOG.debug("field = "+field +",type =" +type+",value="+typeConvert(type,values[i]));
	        			//默认操作字段占用第一个字段
	        			item.put(field, typeConvert(type,values[i+1]));
	        		}
	                items.add(item);
	                insertSize = insertSize + 1;
	                
	                if (txnEventCount == batchSize || insertSize == insertBatchSize){
	                	cl.bulkInsert(items, DBCollection.FLG_INSERT_CONTONDUP);
	                	
	                	items.clear();
	                }
	            }else{
	            	if (operation.equals("I") && operationTypes.contains(operation)){
	            		Object _id = insert(infos[1]); 
		            }else if (operation.equals("U") && operationTypes.contains(operation)){
		            	update(infos[1],infos[2]);
		            }else if (operation.equals("D") && operationTypes.contains(operation)){
		            	delete(infos[1]);
		            }
	            }
	    	}
			LOG.debug("End for floor,the txnEventCount is :"+txnEventCount);
			if (txnEventCount == 0) {
	...
	private Object insert(String content){
		BSONObject item = new BasicBSONObject();
		String[]values = content.split(fieldDelimiter);
        ...
        return cl.insert(item);
	}
	
	private void update(String matcherInfo,String modifierInfo){
		BSONObject matcher = new BasicBSONObject();
    	BSONObject modifier = new BasicBSONObject();
    	String []matchers = matcherInfo.split(",");
    	for (int i = 0; i < matchers.length; i++) {
    		String []item = matchers[i].split(":");
    		matcher.put(item[0], item[1]);
		}
    	
    	BSONObject modifiesObj = new BasicBSONObject();
    	String []modifies = modifierInfo.split(",");
    	for (int i = 0; i < modifies.length; i++) {
    		String []item = modifies[i].split(":");
    		modifiesObj.put(item[0], item[1]);
		}
    	
    	modifier.put("$set", modifiesObj);
    	
    	cl.update(matcher, modifier, null);
	}
	
	private void delete(String matcherInfo){
		BSONObject matcher = new BasicBSONObject();
    	String []matchers = matcherInfo.split(",");
    	for (int i = 0; i < matchers.length; i++) {
    		String []item = matchers[i].split(":");
	...
	private int drainOneBatch(Channel channel) throws BaseException, InterruptedException{
	    ...
	    try {
	    	...
	    	for (; txnEventCount < batchSize; ++txnEventCount) {
	            ...
	            //添加入库时间字段
	            item.put("in_timestamp", System.currentTimeMillis());
	            LOG.debug("item information is :"+item.toString());
